...
public class Lexer {
	...
	public Lexer(String input) {
		...
		position = 0;
		pending = new LinkedList<Token>();	// TODO: is LinkedList the most appropriate here?
	}
	
	public boolean AtEnd() {
		return position >= inputLength && pending.size() == 0;
	}
	...
	public Token Dequeue() throws LexerException {
		if (!pending.isEmpty())
			return pending.poll();
		...
		if (c == '.') {
			...
			if (position >= inputLength || !IsNumeric(input.charAt(position))) {
		...
		if (c == '.' || IsNumeric(c)) {
			...
			while (position < inputLength) {
				if (IsIdentifier(input.charAt(position))) position++;
				else break;
			}
			...
			boolean haveDoubledQuotes = false;
			startPos = position;
			boolean gotEndQuote = false;
			while (position < inputLength) {
				c = input.charAt(position++);
				if (c == '"') {
					if (position < inputLength && input.charAt(position) == '"') {
		...
		// System.out.println(String.format("(%s): %d, %d", input, startPos, position - startPos));
		result.text = input.substring(startPos, position);
		return result;
	}

	void SkipWhitespaceAndComment() {
		while (!AtEnd() && IsWhitespace(input.charAt(position))) {
			position++;
		}
	...
	public boolean IsAtWhitespace() {
		...
		return AtEnd() || IsWhitespace(input.charAt(position));
	}
	
	public static boolean IsInStringLiteral(int charPos, String source) {
		return IsInStringLiteral(charPos, source, 0);
	}

	public static boolean IsInStringLiteral(int charPos, String source, int startPos) {
		boolean inString = false;
		for (int i = startPos; i < charPos; i++) {
			if (source.charAt(i) == '"') inString = !inString;
		}
		return inString;
	}
	...
	public static Token LastToken(String source) throws LexerException {
		...
		while (endPos >= 0 && IsWhitespace(source.charAt(endPos))) endPos--;
		...
		if (IsIdentifier(c)) {
			while (tokStart > startPos && IsIdentifier(source.charAt(tokStart-1))) tokStart--;
		} else if (c == '"') {
			boolean inQuote = true;
			while (tokStart > startPos) {
				tokStart--;
				if (source.charAt(tokStart) == '"') {
					inQuote = !inQuote;
					if (!inQuote && tokStart > startPos && source.charAt(tokStart-1) != '"') break;
				}
			}
		} else if (c == '=' && tokStart > startPos) {
			char c2 = source.charAt(tokStart-1);
	...
	public static void Check(Token tok, Token.Type type) {
		Check(tok, type, null, 0);
	}
	
	public static void Check(Token tok, Token.Type type, String text) {
		Check(tok, type, text, 0);
	}

	public static void Check(Token tok, Token.Type type, String text, int lineNum) {
	...
	public static void RunUnitTests() throws LexerException {
